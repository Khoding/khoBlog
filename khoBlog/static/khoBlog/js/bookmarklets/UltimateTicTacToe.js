...
menu.innerHTML =
  "<a style='background:white;position:absolute;left:100%;cursor:pointer' onClick='menu.remove()'>exit</a>";
...
function check(m) {
  ...
  if (m[0].mark != null) {
    if (
      (m[0].mark == m[1].mark && m[1].mark == m[2].mark) ||
      (m[0].mark == m[3].mark && m[3].mark == m[6].mark)
    ) {
      checkmark = 1;
    }
  }
  ...
  if (m[8].mark != null) {
    if (
      (m[6].mark == m[7].mark && m[7].mark == m[8].mark) ||
      (m[2].mark == m[5].mark && m[5].mark == m[8].mark)
    ) {
      checkmark = 1;
    }
  }
...
function marker(obj) {
  ...
  if (turn == 0) {
    ...
    o.style.border =
      (unit * (obj == board ? 9 : inBOXES ? 3 : 1)) / 5 + "vmin solid black";
